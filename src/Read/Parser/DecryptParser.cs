...
namespace PdfLib.Read.Parser
{
    ...
    internal sealed class DecryptParser : Parser
    {
        ...
        readonly BaseSecurityHandler _sech;

        public PdfEncryption SecuretyHandler { get; private set; }

        #endregion

        #region Init

        internal DecryptParser(Parser parser, PdfStandarSecuretyHandler sech)
            : base(parser)
        { _sech = sech.Handler; SecuretyHandler = sech; }
        ...
        protected override PdfItem ReadObjImpl(PdfType type)
        {
            PdfItem ret;
            if (_num_next != null)
            {
