...
namespace PdfLib.Read.Parser
{
    public class Parser : IIParser
    {
        #region Variables
        ...
        /// </summary>
        bool IIParser.IsClosed { get { return _lex.IsClosed; } }
        ...
        void IIParser.LexPosChanged() { _num_next = null; }
        ...
        void IIParser.Reopen(System.IO.Stream s)
        {
            _lex.Reopen(s);
        }
        ...
        /// </summary>
        PdfTrailer IIParser.ReadTrailer()
        {
        ...
        /// </remarks>
        PdfItem IIParser.ReadItem(PdfType type)
        {
...
#if LONGPDF
        int IIParser.Read(byte[] buffer, long offset)
#else
        ...
        /// <returns>How many bytes were copied</returns>
#if LONGPDF
        int IIParser.Read(byte[] buffer, long stream_off, int count)
#else
        ...
        /// Gets a raw string.
        /// </summary>
        string IIParser.GetDebugString(int start, int length)
        {
        ...
        /// 
        /// This function puts aside one value (a number), and use the peek functionality 
        /// in the lexer to "look" at three values simultaniously.
        /// 
        /// The cached data is a such:
        ///  1. Current value
        ///  2. Next number (if numeric)
        ///  3. The current calue of the lexer. 
        /// </remarks>
        protected virtual PdfItem ReadObjImpl(PdfType type)
        {
            ...
            if (_num_next != null)
            {
                //The object is "read", should always be either a integer or a real
                ret = _num_next;
                _num_next = null;

                //Checks if it's the expected type, this should never fail.
        ...
        check_if_ref:
            ...
            //Note that "peek" can't differentiate between a real
            //and a integer. 
            ...
            if (new_type != PdfType.Integer)
            {
                ...
                //(since we know it's a number)
                //but since we read it in we need to hold on to the 
                //Real.
                _num_next = ret;
            ...
            //Putting aside the original integer.
            _num_next = num;
            ...
            if (_lex.IsR())
            {
                ...
                if (pdf_ref == null)
                {
                    ...
                    { //Note, PdfFile now always return references during the construction phase,
                      ...
                      //      (I.e. this code will always execute.)
                    ...
                    // Note, this code will never execute.

                    //This might seem harmful, because we can now have two sepperate instances of the reference to the
                    //same PDF object. However PdfLib wasn't designed to make the references themselves unique, only the 
                    //objects they reference.
                    //
                    //The alternative is probably to call _owner.RegisterObject(id, 0) followed by _owner.GetReference.
                    //
                    //Note that there's no need to use RWReference, even for writable documents. It would be better,
                    //as peeking on references is sometimes used to check if an object must be copied, but it won't 
                    //break anything, since the reference dosn't actually determine if an object is writable.
                    //
                    //If the id is never registered, then this reference will simply point at PdfNull, as per the specs.
        ...
        private PdfObject ReadDictImpl()
        {
            ...
            if (_lex.PeekNextKeyword(Chars.s, Chars.t, PdfKeyword.BeginStream))
            {
                ...
                if (!cat.TryGetValue("Length", out l))
                    //Should perhaps instead search for "EndStream"
                ...
                {
                    //Looks for the endstrean keyword in a range of +/- 1000 bytes.
                    byte[] search = new byte[2000];
                    _lex.Read(search, range.Start + Math.Max(0, range.Length - 1000), search.Length);
                    string txt = Lexer.GetString(search);
                    int idx = txt.IndexOf("endstream");
                    if (idx != -1)
                    {
                        long real_length = Math.Max(0, range.Length - 1000) + idx;
                        cat["Length"] = new PdfInt((int) real_length);
                        range = new rRange(range.Start, (int) real_length);
                        _lex.Position = range.Start + real_length;
                        SetNextKeyword(PdfKeyword.EndStream);
                    }
                    else
                        throw pe;
        ...
        Catalog IIParser.ReadCatalog() { return ReadCatalog(); }
        ...
        internal static PdfObject CreatePdfType(PdfDictionary dict, PdfType t, IntMsg msg, object obj)
        {
            switch (t)
            {
                ...
                case PdfType.CryptFilter:
                    return new PdfCryptFilter(dict);

                case PdfType.CryptFilters:
                    return new PdfCryptFilters(dict);
                ...
                case PdfType.DestDictionary:
                    return new PdfDestDictionary(dict);
