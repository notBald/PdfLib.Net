...
namespace PdfLib.Read.TrueType
{
    ...
    /// 
    /// There's "build" table and there's "normal" (read) tables. What
    /// is the difference?
    /// 
    /// Currently all the build tables are tucked into BuildTables.cs.
    /// These tables were created after the read tables, so they parse
    /// a little faster and allows for modifying the data.
    /// 
    /// There's no reason to use a read table over a build table beyond
    /// that the build tables doesn't support all lookup features* and switching
    /// to them would mean modifying working code. I.e. the build tables are 
    /// better but are only used when building tables for now. 
    /// 
    /// * and one can't validate them. That should be implemented.
    /// </remarks>
    public class TableDirectory : IDisposable
    {
        #region Variables
        ...
        /// <summary>
        /// If this is a compact font format file
        /// </summary>
        public bool IsCFF { get { return GetOptTable(Tag.CFF) != null; } }
        ...
        /// <summary>
        /// Embeded compact font format font
        /// </summary>
        public byte[] CFF
        {
            get
            {
                var td = GetTable(Tag.CFF);

                long org = _f.Position;
                _f.Position = td.Offset;
                var bytes = new byte[(int)td.Length];
                if (_f.Read(bytes, 0, bytes.Length) != bytes.Length)
        ...
        /// </summary>
        /// <remarks>
        /// Using a build table as there was never a read table 
        /// for this.
        /// 
        /// Only drawback is that it resets the defaultchar
        /// to 0 and it can't be validated with "Valid"*
        /// 
        /// * This should be easy to implement though as
        ///   one has all the data. Just make sure the
        ///   checksum algo don't barf on short tables
        /// </remarks>
        internal OS2Build OS2
        {
            get
            {
                var td = GetTableData(Tag.OS2, false);
                if (td == null) return null;
                return new OS2Build(td);
            }
        }
        ...
        /// <remarks>
        /// Using a build table as there was never a read table 
        /// for this. Only drawback is that one can't validate
        /// the table with "Valid".
        /// </remarks>
        internal PostBuild Post
        {
            get
            {
                var td = GetTableData(Tag.post, false);
                if (td == null) return null;
                return new PostBuild(td);
            }
        }
        ...
        internal NameTable Name
        {
            get
            {
                var td = GetTable(Tag.name);

                long org = _f.Position;
                _f.Position = td.Offset;
                var ret = new NameTable(this, _f);
                _f.Position = org;
        ...
        internal KernTable Kern
        {
            get
            {
                var td = GetOptTable(Tag.kern);
                if (td == null) return null;

                long org = _f.Position;
                _f.Position = td.Offset;
                var ret = new KernTable(this, _f);
                _f.Position = org;
                td.Table = ret;
        ...
        /// <summary>
        /// Only for when one wish to create an induvidual table
        /// </summary>
        /// <param name="s">Stream reader</param>
        internal TableDirectory(StreamReaderEx s)
        {
            _f = s;
        }
        ...
        /// <summary>
        /// Fetches the raw data of the table
        /// </summary>
        /// <remarks>This is used when creating embeded fonts</remarks>
        internal byte[] GetTableData(Tag tag, bool required)
        {
            var td = required ? GetTable(tag) : GetOptTable(tag);
            if (td == null) return null;
            var org = _f.Position;
            _f.Position = td.Offset;
            var data = new byte[td.Length];
            _f.Read(data, 0, data.Length);
            _f.Position = org;
            return data;
        }
        ...
        internal bool IsValid(Tag tag)
        {
            ...
            //The specs also takes along the pad bytes, but what to
            //do when the table is at the end of the font?
            end = td.Length - end * 4;
            for (int c = 0; c < end; c++)
        ...
        /// <summary>
        /// Checks if the file is a True Type font
        /// </summary>
        /// <param name="s">File to check</param>
        /// <returns>True if true type</returns>
        public static bool IsTTF(Stream s)
        {
            ...
            try
            {
                ...
                try
                {
                    var _f = new StreamReaderEx(s);

                    //Reads out the table directory from the start of the stream
                    var _version = ReadFixed(_f);
                    int n_tables = _f.ReadUShort();
    ...
    enum Tag : uint
    {
        ...
        GSUB = 1196643650,
        /// <summary>
        /// Linear Threshold (OpenType table)
        ...
        /// http://partners.adobe.com/public/developer/opentype/index_vdmx.html
        /// </summary>
        VDMX = 1447316824,
        /// <summary>
        /// Character map
        ...
        /// </summary>
        /// <remarks>
        /// Apple recomends that this should be stripped from the font before sending
        /// it to the user as the information within is related to the tool used to
        /// create the font.
        /// </remarks>
        edt0 = 1701082160,
        /// <summary>
