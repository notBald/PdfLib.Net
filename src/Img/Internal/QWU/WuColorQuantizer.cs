...
namespace PdfLib.Img.Internal.QWU
{
    ...
    internal sealed class WuColorQuantizer : IColorQuantizer
    {
        /// <summary>
        /// The index bits.
        /// </summary>
        ...
        /// Quantizes an image.
        /// </summary>
        /// <param name="image">The BGR24 image).</param>
        ...
        {
            var quantizedImage = new ColorQuantizerResult(image.Length / 3, colorCount, 3);
            ...
            for (int k = 0; k < colorCount; k++)
            {
                ...
                if (weight != 0)
                {
                    quantizedImage.Palette[(k * 3)] = (byte)(WuColorQuantizer.Volume(cube[k], this.vmr) / weight);
                    quantizedImage.Palette[(k * 3) + 1] = (byte)(WuColorQuantizer.Volume(cube[k], this.vmg) / weight);
                    quantizedImage.Palette[(k * 3) + 2] = (byte)(WuColorQuantizer.Volume(cube[k], this.vmb) / weight);
                }
                else
                {
                    quantizedImage.Palette[(k * 3) + 2] = 0;
                    quantizedImage.Palette[(k * 3) + 1] = 0;
                    quantizedImage.Palette[k * 3] = 0;
                }
            }

            for (int i = 0; i < image.Length / 3; i++)
            {
                int r = image[(i * 3) + 2] >> (8 - WuColorQuantizer.IndexBits);
                int g = image[(i * 3) + 1] >> (8 - WuColorQuantizer.IndexBits);
                int b = image[i * 3] >> (8 - WuColorQuantizer.IndexBits);
