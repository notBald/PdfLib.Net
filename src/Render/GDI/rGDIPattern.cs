...
{
    /// <summary>
    /// Basically a copy/paste of rCairoPattern, with adaptations for GDI.
    /// 
    /// Note: Made it draw using a "Cairo" like interface (iGraphics). 
    /// </summary>
    internal static class rGDIPattern
    {
        internal interface iGraphics
        {
            void SetColor(DblColor col);
            void CircleAt(double centerX, double centerY, double radius);
            void DrawCircle(double centerX, double centerY, double radius);
            void RectangleAt(xRect r);
            void RectangleAt(double x, double y, double width, double height);
            void DrawRectangle(xRect r);
            void DrawRectangle(double x, double y, double width, double height);
            void DrawLine(xPoint from, xPoint to, double width);
            ...
            void Transform(xMatrix m);
        }

        public static void RenderShade(PdfShading shade, iGraphics dc, xRect bounds)
        {
            if (shade is PdfAxialShading)
                rGDIPattern.RenderShade((PdfAxialShading)shade, dc, bounds, new xVector());
            else if (shade is PdfRadialShading)
                rGDIPattern.RenderShade((PdfRadialShading)shade, dc, bounds);
            else if (shade is PdfFunctionShading)
                rGDIPattern.RenderShade((PdfFunctionShading)shade, dc, bounds);
        }
        ...
        ///                               t = t0 + dt * x¢
        /// </remarks>
        public static void RenderShade(PdfAxialShading shade, iGraphics dc, xRect bounds, xVector px_size)
        {
            #region Step A
        ...
        /// </remarks>
        public static void RenderShade(PdfRadialShading shade, iGraphics dc, xRect bounds)
        {
        ...
        /// </remarks>
        public static void RenderShade(PdfFunctionShading shade, iGraphics dc, xRect bounds)
        {
